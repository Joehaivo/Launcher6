...
public class PackageInstallerCompatVL extends PackageInstallerCompat {
    public static final String TAG = PackageInstallerCompatVL.class.getSimpleName();
    ...
    public HashMap<String, SessionInfo> updateAndGetActiveSessionCache() {
        ...
        Log.d(TAG, "updateAndGetActiveSessionCache: activePackages=" + activePackages.size());
        return activePackages;
    }

    @Thunk
    void addSessionInfoToCache(SessionInfo info, UserHandle user) {
        Log.d(TAG, "addSessionInfoToCache: info=" + info + ", user=" + user);
    ...
    private final SessionCallback mCallback = new SessionCallback() {
        ...
        public void onCreated(int sessionId) {
            ...
            Log.d(TAG, "SessionCallback onCreated: sessionId=" + sessionId + ", sessionInfo=" + sessionInfo);
...
//            List<PackageInstaller.SessionInfo> allPackageInstallerSessions = mLauncherApps.getAllPackageInstallerSessions();
//            Log.d(TAG, "pushSessionDisplayToLauncher: allPackageInstallerSessions " + allPackageInstallerSessions.size());
            SessionInfo session = verify(sessionInfo);
            Log.d(TAG, "pushSessionDisplayToLauncher: sessionId=" + sessionId + ", sessionInfo=" + sessionInfo + ", session=" + session);
            if (session != null && session.getAppPackageName() != null) {
    ...
    private PackageInstaller.SessionInfo verify(PackageInstaller.SessionInfo sessionInfo) {
        Log.d(TAG, "verify: sessionInfo=" + sessionInfo);
        if (sessionInfo == null) {
            return null;
        }
        String installerPackageName = sessionInfo.getInstallerPackageName();
        String appPackageName = sessionInfo.getAppPackageName();
        Log.d(TAG, "verify: installerPackageName=" + installerPackageName + ", appPackageName=" + appPackageName);
        if (installerPackageName == null || TextUtils.isEmpty(appPackageName)) {
            return null;
        }
        Log.d(TAG, "verify: pkg " + installerPackageName);
        synchronized (mSessionVerifiedMap) {
            if (!mSessionVerifiedMap.containsKey(installerPackageName)) {
                LauncherAppsCompat launcherApps = LauncherAppsCompat.getInstance(mAppContext);
                boolean hasSystemFlag = launcherApps.getApplicationInfo(installerPackageName,
                        ApplicationInfo.FLAG_SYSTEM, Process.myUserHandle()) != null;
                mSessionVerifiedMap.put(installerPackageName, DEBUG);
            }
        }
        return sessionInfo;
    }
